...
namespace otto
{
    namespace
    {
        ...
        struct _Component
        {
            ...
            bool8 operator==(const _Component& other) const { return relativeFilePath == other.relativeFilePath; }
        };
        ...
        struct _Event
        {
            ...
            bool8 operator==(const FilePath& filePath) const { return relativeFilePath == filePath; }
            bool8 operator==(const _Event& other) const { return relativeFilePath == other.relativeFilePath; }
        };
        ...
        static FilePath _getPackageFilePath(const FilePath& filePath)
        {
            return filePath.toString().startsWith(FilePath("otto/").toString()) ? 
                "C:/dev/Otto/Otto/src/packages/" + filePath + ".hpp" : Application::getRootDirectory() + filePath + ".hpp";
        }
        ...
        DynamicArray<_Component> _loadComponents(const DynamicArray<String> components)
        {
            ...
            for (auto& component : components)
            {
                ...
                FilePath filePath = _getPackageFilePath(component);
        ...
        DynamicArray<_Behaviour> _loadBehaviours(const DynamicArray<String> behaviours, const DynamicArray<_Event>& loadedEvents)
        {
            ...
            for (auto& behaviour : behaviours)
            {
                ...
                FilePath filePath = _getPackageFilePath(behaviour);
            ...
            const DynamicArray<_Component>& loadedComponents, const DynamicArray<_Event>& loadedEvents)
        {
            ...
            for (auto& system : systems)
            {
                ...
                FilePath filePath = _getPackageFilePath(system);
                ...
                if (!functions.hasValue())
                {
                    Log::error("Error in behaviour file ", filePath, ": File does not contain class ", loadedSystem.name);
                    continue;
                }
        ...
        void _addComponentAndEventIncludes(String& code, const _Package& package)
        {
            for (auto& component : package.components)
                code.append("#include \"" + _getPackageFilePath(component.relativeFilePath).toString() + "\"\n");
            for (auto& e : package.events)
                code.append("#include \"" + _getPackageFilePath(e.relativeFilePath).toString() + "\"\n");
        ...
        void _addBehaviourAndSystemIncludes(String& code, const _Package& package)
        {
            for (auto& behaviour : package.behaviours)
                code.append("#include \"" + _getPackageFilePath(behaviour.relativeFilePath).toString() + "\"\n");
            for (auto& system : package.systems)
                code.append("#include \"" + _getPackageFilePath(system.relativeFilePath).toString() + "\"\n");
        ...
        void _addSceneCoreFunctions(String& code, const _Package& package)
        {
            ...
            if (package.events.contains("otto/core/events/InitEvent"))
            ...
            if (package.events.contains("otto/core/events/UpdateEvent"))
                code.append("        dispatchEvent(UpdateEvent(delta));\n");
            ...
            code.append("    {\n");
            code.append("        Window::clear();\n");

            if (package.events.contains("otto/core/events/RenderEvent"))
                code.append("        dispatchEvent(RenderEvent());\n");

            code.append("        Window::swapBuffers();\n");
            code.append("    }\n");
        ...
        void _addSceneOnEventFunctions(String& code, const _Package& package)
        {
            for (auto& e : ON_EVENT_FUNCTIONS)
            {
                code.append("    OTTO_DLL_FUNC void Scene::_on" + e + "(const _" + e + "Event& e)\n");
                code.append("    {\n");

                if (package.events.contains("otto/window/events/" + e + "Event"))
                    code.append("        dispatchEvent(e);\n");
        ...
        void _createSceneFile(const _Package& package, const FilePath& filePath)
        {
            ...
            code.append("#include \"packages/otto/core/events/ComponentAddedEvent.hpp\"\n");
            code.append("#include \"packages/otto/core/events/ComponentRemovedEvent.hpp\"\n");
            code.append('\n');
            ...
            _addSceneCoreFunctions(code, package);
            ...
            _addSceneOnEventFunctions(code, package);
